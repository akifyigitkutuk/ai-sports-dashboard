...
export default function ActionLog({ events }: Props) {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '0.6rem', fontWeight: 800, letterSpacing: '2px', textTransform: 'uppercase', color: '#888', marginBottom: '4px', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '4px' }}>
        Live Action History
      </p>
      ...
      {[...events].reverse().map((ev, i) => {
        ...
        return (
          <div key={ev.id || i} style={{
            ...
            <span style={{ fontSize: '0.6rem', fontWeight: 800, color: '#666', minWidth: '24px' }}>
              [{ev.minute}&apos;]
            </span>
            <span style={{ height: '5px', width: '5px', borderRadius: '50%', backgroundColor: dotColor, boxShadow: `0 0 6px ${dotColor}` }} />
            <span style={{ fontSize: '0.65rem', fontWeight: 700, color: isMiss ? '#ff4b4b' : '#eee', flex: 1 }}>
              {ev.type} <span style={{ opacity: 0.5, fontSize: '0.58rem' }}>({ev.team === 0 ? 'Home' : 'Away'})</span>
            </span>
            
            {isHit && ev.latency !== undefined && (
              <span style={{ fontSize: '0.58rem', fontWeight: 800, color: '#00e676', opacity: 0.8 }}>
                ✓ {Math.round(ev.latency)}ms
              </span>
            )}
            {isMiss && (
              <span style={{ fontSize: '0.58rem', fontWeight: 900, color: '#ff4b4b' }}>
                MISSED 🚨
              </span>
            )}
          </div>
